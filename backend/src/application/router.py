...
from ..domain.entities import County, RiskFactorReport, MunicipalIndicatorsReport, MunicipalResilienceProfileReport, ClimateProjectionReport, MunicipalHealthReport

from ..domain.interfaces import  PdfServiceInterface, ProjectInfoServiceInterface
from ..domain.interfaces import CountyRepositoryInterface, RiskFactorRepositoryInterface, MunicipalIndicatorsRepositoryInterface, MunicipalResilienceProfileRepositoryInterface, ClimateProjectionRepositoryInterface, MunicipalHealthRepositoryInterface

from .dependencies import get_pdf_service, get_project_info_service
from .dependencies import get_county_repository, get_risk_factor_repository, get_municipal_report_repository, get_municipal_resilience_profile_repository, get_climate_projection_repository, get_municipal_health_repository
...
async def download_report_pdf(
    ...
    municipal_health_repo: MunicipalHealthRepositoryInterface = Depends(get_municipal_health_repository),
    ...
    print(f"\n--- Fetching data for county_id: {county_id} ---")
    ...
    municipal_health_data = await municipal_health_repo.get_municipal_health(county_id)
    ...
    if not municipal_health_data:
        raise HTTPException(status_code=404, detail=ErrorKeys.MUNICIPAL_HEALTH_NOT_FOUND.value)
    ...
    municipal_health_record = MunicipalHealthReport(municipal_health=municipal_health_data).formatted_data_dict
    ...
    context = {
        ...
        "municipal_health_record": municipal_health_record,
    ...
    try:
        ...
        print(f"\n--- Generating PDF Report for county_id: {county_id} ---")
...
async def download_report_page_pdf(
    ...
    # Print initial log to confirm endpoint is hit
    print(f"---"*30)
    print(f"--- Single Page PDF Report Request ---")
    print(f"--- Received request for page '{page_name}' of county_id: {county_id} from IP: {request.client.host}")
    ...
    if "municipal_health_record" in required_records:
        municipal_health_data = await municipal_health_repo.get_municipal_health(county_id)
        if not municipal_health_data:
            raise HTTPException(status_code=404, detail=ErrorKeys.MUNICIPAL_HEALTH_NOT_FOUND.value)
        context["municipal_health_record"] = MunicipalHealthReport(municipal_health=municipal_health_data).formatted_data_dict
    ...
    try:
        ...
        pdf_bytes = await pdf_service.generate_pdf_page(context, page_name)
        print(f"--- Single Page PDF Report Generated ---")
        print(f"--- Successfully generated PDF page '{page_name}' for county_id: {county_id}, sending response...")
    except KeyError:
        raise HTTPException(status_code=404, detail=ErrorKeys.PAGE_NOT_FOUND.value)
    except Exception as e:
        print(f"Error generating PDF: {ErrorKeys.PDF_GENERATION_FAILED.value} {e}")
